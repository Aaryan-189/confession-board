  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center py-12 px-4 sm:px-6">
      <div className="w-full max-w-2xl space-y-8">
        ...
        <form onSubmit={handleSubmit} className="bg-zinc-950 border border-zinc-900 rounded-2xl p-4 sm:p-6 backdrop-blur-xl shadow-2xl space-y-4">
          <textarea
            value={content}
            ...
            className="w-full bg-black border border-zinc-900 rounded-xl p-3 text-sm text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-emerald-500 transition-colors resize-none"
          />
          {error && <p className="text-xs text-rose-400">{error}</p>}
          <div className="flex items-center justify-between">
            <span className={`text-xs ${content.length > 220 ? 'text-amber-400' : 'text-zinc-600'}`}>
              {content.length}/250
            </span>
            <button
              type="submit"
